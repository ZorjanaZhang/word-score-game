...
function endGame(){
	console.log("left unused words:"+BAG_OF_LETTERS.length);
	if(BAG_OF_LETTERS.length<=0){
		alert('Game Over!The highest score is'+SCORE);
	}
}
...
function findWordToUse(){
 //TODO Your job starts here.
	...
	humanFoundWord= $( "#human-word-input").val();
	if(humanFoundWord.indexOf('_')>-1){
		var privateList=[humanFoundWord];
		let arr=humanFoundWord.split('');
		for(let i in arr){
			let size=privateList.length;
			for(let j=0;j<size;j++){
				if(arr[i]=="_"){
					for(let m=97;m<123;m++){
						//造字母
						let str=String.fromCharCode(m);
						let privateListNew = privateList[j].substring(0,i).concat(str,privateList[j].substring(parseInt(i)+1));
						if(privateListNew.indexOf('_')==-1){
							let status=humanFindWordToUse(privateListNew);
							if(status){
								return true;
							}
						}
		...
		let status=humanFindWordToUse(humanFoundWord);
		if(!status){
			alert(humanFoundWord + " - Do not have the letters for this word");
		}
	}
}
function humanFindWordToUse(checkWords){
	
	 var humanFoundWord = $( "#human-word-input").val();
	//  console.log("Checking human workd of:" + humanFoundWord);
	 if(isThisAWord(checkWords)){
